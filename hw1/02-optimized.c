...
int main(int argc, char *argv[])
{
	...
	double * coords_0 = coords[0];
	double * coords_1 = coords[1];
	double * coords_2 = coords[2];
	...
	for (i=1; i<=natom; ++i)
	{
		...
		for (j=1; j<=natom; ++j)
		{
			...
			{
				vec2 = pow((c0i-coords_0[j-1]),2.0)
					+ pow((c1i-coords_1[j-1]),2.0)
					+ pow((c2i-coords_2[j-1]),2.0);
				/* X^2 + Y^2 + Z^2 */
				rij = sqrt(vec2);
				...
				if ( rij <= cut )
				{
					...
					++cut_count;
					current_e = (exp(rij*q_i)*exp(rij*q[j-1]))/rij;
