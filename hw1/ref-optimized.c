...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < natom; ++i)
	{
		...
		for (j = 0; j < i; ++j)
		{
			vec2 = (i0-coords_0[j])*(i0-coords_0[j]) //splitting it into vec2_1 didnt do anything
			...
			if ( vec2 <= cut2 ) /* we're now just directly comparing vec2 to the square of the cutoff */
			{
